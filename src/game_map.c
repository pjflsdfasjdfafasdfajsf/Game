...
bool map_load(memory_arena *permanent_arena, memory_arena *temporary_arena, memory_stream *error_stream, const void *data, usize size, map *map)
{
    ...
    if (wall_count > ARRAY_COUNT(map->walls))
    {
        memory_stream_write_string_format(error_stream, "error: wall count %u exceeds maximum %u. This might be a corrupted map\n", wall_count, ARRAY_COUNT(map->walls));
        return false;
    }
